...
void print_idxs(int A1_size, int *A2_pos, int *A2_idx, double *A_vals, double *y_vals, double *x_vals) {
  // Generated by the Tensor Algebra Compiler (tensor-compiler.org)
  for (int32_t iA = 0; iA < A1_size; iA++) {
    double tj = 0;
    for (int32_t pA2 = A2_pos[iA]; pA2 < A2_pos[iA + 1]; pA2++) {
      int32_t jA = A2_idx[pA2];
			std::cout << iA << ',' << jA << ',' << pA2 << ',' << jA << '\n';
...
int main() {
  ...
  print_idxs(A1_size, A2_pos, A2_idx, A_vals, y_vals, x_vals);
}
